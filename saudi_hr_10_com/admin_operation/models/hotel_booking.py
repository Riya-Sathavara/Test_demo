...
class TicketType(models.Model):
    ...
    name = fields.Char('Types of Hotels')
...
class HotelBooking(models.Model):
    ...
    travel_purpose_id = fields.Many2one('travel.purpose', string='Purpose Of Hotel')
    ...
    hotel_state = fields.Many2one('res.country.state', 'State')
    ...
    traveling_date_from = fields.Date('Stay From Date', required=True)
    traveling_date_to = fields.Date('Stay To Date', required=True)
    ...
    hr_expense_ids = fields.One2many('hr.expense', 'hotel_booking_id', string='Expense')
    ir_attachment_ids = fields.One2many('ir.attachment', 'hotel_book_id', string='Expense')
    
    @api.multi
    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'state' in init_values and self.state == 'waiting_for_approval':
            return 'admin_operation.mt_hotel_booking_confirmed'
        elif 'state' in init_values and self.state == 'validate':
            return 'admin_operation.mt_hotel_booking_validated'
        elif 'state' in init_values and self.state == 'approved':
            return 'admin_operation.mt_hotel_booking_approved'
        elif 'state' in init_values and self.state == 'refuse':
            return 'admin_operation.mt_hotel_booking_refused'
        elif 'state' in init_values and self.state == 'cancel':
            return 'admin_operation.mt_hotel_booking_canceled'
        return super(HotelBooking, self)._track_subtype(init_values)
    ...
    def default_get(self, fields):
        rec = super(HotelBooking, self).default_get(fields)
