...
class FlightBooking(models.Model):
    ...
    ir_attachm_ids = fields.One2many('ir.attachment', 'flight_book_id', string='Expense')
    
    @api.multi
    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'state' in init_values and self.state == 'waiting_for_approval':
            return 'admin_operation.mt_flight_booking_confirmed'
        elif 'state' in init_values and self.state == 'validate':
            return 'admin_operation.mt_flight_booking_validated'
        elif 'state' in init_values and self.state == 'approved':
            return 'admin_operation.mt_flight_booking_approved'
        elif 'state' in init_values and self.state == 'refuse':
            return 'admin_operation.mt_flight_booking_refused'
        elif 'state' in init_values and self.state == 'cancel':
            return 'admin_operation.mt_flight_booking_canceled'
    ...
    def default_get(self, fields):
        rec = super(FlightBooking, self).default_get(fields)
    ...
    def create(self, vals):
        ...
        return super(FlightBooking, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(FlightBooking, self).write(vals)
